...
from ..analysis.comparison.hist_comparer import ExpandingHistComparer, \
    RollingHistComparer, ReferenceHistComparer, PreviousHistComparer
from ..analysis.profiling.pull_calculator import RollingPullCalculator, \
...
from ..alerting import DynamicBounds, TrafficLightAlerts, traffic_light_summary, StaticBounds, AlertsSummary


def metrics_self_reference(hists_key='test_hists', time_axis='date', window=10, monitoring_rules={}, pull_rules={},
    ...
    modules = [
        ...
        HistSplitter(read_key=hists_key, store_key='split_hists', features=features,
        ...
        RefMedianMadPullCalculator(reference_key='comparisons', assign_to_key='comparisons',
                                   suffix_mean='_mean', suffix_std='_std', suffix_pull='_pull',
        ...
        RefMedianMadPullCalculator(reference_key='profiles', assign_to_key='profiles',
                                   suffix_mean='_mean', suffix_std='_std', suffix_pull='_pull'),
        # --- 5. looking for significant rolling linear trends in selected features/metrics
        ApplyFunc(apply_to_key='profiles', assign_to_key='comparisons',
                  apply_funcs=[dict(func=rolling_lr_zscore, suffix='_trend{w}_zscore'.format(w=window),
                                    entire=True, window=window, metrics=['mean', 'phik', 'fraction_true'])],
        ...
        StaticBounds(read_key='profiles', rules=pull_rules, store_key='dynamic_bounds',
        ...
        TrafficLightAlerts(read_key='profiles', rules=monitoring_rules,
                           store_key='traffic_lights', expanded_rules_key='static_bounds'),
        TrafficLightAlerts(read_key='comparisons', rules=monitoring_rules,
                           store_key='traffic_lights', expanded_rules_key='static_bounds_comparisons'),
        ApplyFunc(apply_to_key='traffic_lights', apply_funcs=[dict(func=traffic_light_summary, axis=1, suffix='')],
                  assign_to_key='alerts', msg='Generating traffic light alerts summary.'),
...
def metrics_external_reference(hists_key='test_hists', ref_hists_key='ref_hists', time_axis='date', window=10,
                               monitoring_rules={}, pull_rules={}, features=None, **kwargs):
    ...
    modules = [
        ...
        HistSplitter(read_key=ref_hists_key, store_key='split_ref_hists', features=features,
                     feature_begins_with=f'{time_axis}:'),
        ReferenceHistComparer(reference_key='split_ref_hists', assign_to_key='split_hists', store_key='comparisons'),
        RefMedianMadPullCalculator(reference_key='comparisons', assign_to_key='comparisons',
                                   suffix_mean='_mean', suffix_std='_std', suffix_pull='_pull',
                                   metrics=['ref_max_prob_diff']),
        ...
        ReferencePullCalculator(reference_key='ref_profiles', assign_to_key='profiles',
                                suffix_mean='_mean', suffix_std='_std', suffix_pull='_pull'),
        # --- 5. looking for significant rolling linear trends in selected features/metrics
        ApplyFunc(apply_to_key='profiles', assign_to_key='comparisons',
                  apply_funcs=[dict(func=rolling_lr_zscore, suffix='_trend{w}_zscore'.format(w=window),
...
def metrics_rolling_reference(hists_key='test_hists', time_axis='date', window=10, shift=1,
                              monitoring_rules={}, pull_rules={}, features=None, **kwargs):
    ...
    modules = [
        ...
        RollingHistComparer(read_key='split_hists', window=window, shift=shift, store_key='comparisons'),
        RefMedianMadPullCalculator(reference_key='comparisons', assign_to_key='comparisons',
                                   suffix_mean='_mean', suffix_std='_std', suffix_pull='_pull',
        ...
        #        to obtain normalized residuals of profiles
        HistProfiler(read_key='split_hists', store_key='profiles'),
        RollingPullCalculator(read_key='profiles', window=window, shift=shift,
                              suffix_mean='_mean', suffix_std='_std', suffix_pull='_pull'),
        # --- 5. looking for significant rolling linear trends in selected features/metrics
        ApplyFunc(apply_to_key='profiles', assign_to_key='comparisons',
                  apply_funcs=[dict(func=rolling_lr_zscore, suffix='_trend{w}_zscore'.format(w=window),
        ...
        # --- 6. generate dynamic traffic light boundaries, based on traffic lights for normalized residuals,
        #        used for plotting in popmon_profiles report.
        DynamicBounds(read_key='profiles', rules=pull_rules, store_key='dynamic_bounds',
                      suffix_mean='_mean', suffix_std='_std'),
        DynamicBounds(read_key='comparisons', rules=pull_rules, store_key='dynamic_bounds_comparisons',
                      suffix_mean='_mean', suffix_std='_std'),
        # --- 7. expand all (wildcard) static traffic light bounds and apply them.
        #        Applied to both profiles and comparisons datasets
        TrafficLightAlerts(read_key='profiles', rules=monitoring_rules,
...
def metrics_expanding_reference(hists_key='test_hists', time_axis='date', window=10, shift=1,
                                monitoring_rules={}, pull_rules={}, features=None, **kwargs):
    ...
    modules = [
        ...
        # --- 3. profiling of reference histograms, then comparison of with profiled test histograms
        #        results in chi2 comparison of histograms
        ExpandingHistComparer(read_key='split_hists', shift=shift, store_key='comparisons'),
        # --- 4. profiling of histograms, then pull calculation compared with reference mean and std,
        #        to obtain normalized residuals of profiles
        RefMedianMadPullCalculator(reference_key='comparisons', assign_to_key='comparisons',
                                   suffix_mean='_mean', suffix_std='_std', suffix_pull='_pull',
        ...
        HistProfiler(read_key='split_hists', store_key='profiles'),
        ExpandingPullCalculator(read_key='profiles', shift=shift,
                                suffix_mean='_mean', suffix_std='_std', suffix_pull='_pull'),
        # --- 5. looking for significant rolling linear trends in selected features/metrics
        ApplyFunc(apply_to_key='profiles', assign_to_key='comparisons',
                  apply_funcs=[dict(func=rolling_lr_zscore, suffix='_trend{w}_zscore'.format(w=window),
