...
from ...analysis.functions import pull, expanding_mean, expanding_std, rolling_mean, rolling_std
...
class PullCalculator(Pipeline):
    ...
    """
    def __init__(self, func_mean, func_std, apply_to_key, assign_to_key=None, store_key=None,
        ...
        calc_mean_std = ApplyFunc(apply_to_key=apply_to_key, assign_to_key=assign_to_key,
                                  features=features)
        calc_mean_std.add_apply_func(func_std, suffix=suffix_std, entire=True, *args, **kwargs)
        calc_mean_std.add_apply_func(func_mean, suffix=suffix_mean, entire=True, *args, **kwargs)

        calc_pull = ApplyFunc(apply_to_key=assign_to_key, assign_to_key=store_key, features=features)
        calc_pull.add_apply_func(pull, suffix=suffix_pull, axis=1, suffix_mean=suffix_mean, suffix_std=suffix_std)
...
class RollingPullCalculator(PullCalculator):
    ...
    """
    def __init__(self, read_key, window, shift=1, features=None, store_key=None, suffix_mean='_roll_mean',
                 suffix_std='_roll_std', suffix_pull='_roll_pull', *args, **kwargs):
        """Initialize an instance of HistComparer.
        ...
        kws.update(kwargs)
        super().__init__(rolling_mean, rolling_std, read_key, read_key, store_key,
                         suffix_mean, suffix_std, suffix_pull, features, *args, **kws)
        self.read_key = read_key
        self.window = window

    def transform(self, datastore):
        self.logger.info(f'Comparing \"{self.read_key}\" with rolling mean/std of {self.window} past values.')
...
class ExpandingPullCalculator(PullCalculator):
    ...
    """
    def __init__(self, read_key, shift=1, features=None, store_key=None, suffix_mean='_exp_mean',
                 suffix_std='_exp_std', suffix_pull='_exp_pull', *args, **kwargs):
        """Initialize an instance of HistComparer.
        ...
        kws = {'shift': shift}
        kws.update(kwargs)
        super().__init__(expanding_mean, expanding_std, read_key, read_key, store_key,
                         suffix_mean, suffix_std, suffix_pull, features, *args, **kws)
        self.read_key = read_key

    def transform(self, datastore):
        self.logger.info(f'Comparing \"{self.read_key}\" with expanding mean/std of past values.')
        return super().transform(datastore)
...
class ReferencePullCalculator(PullCalculator):
    ...
    """
    def __init__(self, reference_key, assign_to_key, store_key=None, features=None, suffix_mean='_ref_mean',
                 suffix_std='_ref_std', suffix_pull='_ref_pull', *args, **kwargs):
        """Initialize an instance of HistComparer.
        ...
        :param kwargs: (dict, optional): residual kwargs passed on to mean and std functions
        """
        super().__init__(np.mean, np.std, reference_key, assign_to_key, store_key,
                         suffix_mean, suffix_std, suffix_pull, features, *args, **kwargs)
...
class RefMedianMadPullCalculator(PullCalculator):
    ...
    def __init__(self, reference_key, assign_to_key, store_key=None, features=None, suffix_mean='_ref_mean',
        ...
        def df_median(df):
            ...
            return df.median()
        super().__init__(df_median, mad, reference_key, assign_to_key, store_key,
                         suffix_mean, suffix_std, suffix_pull, features, *args, **kwargs)
        self.reference_key = reference_key
        self.assign_to_key = assign_to_key

    def transform(self, datastore):
        self.logger.info(f'Comparing \"{self.assign_to_key}\" with median/mad of reference \"{self.reference_key}\"')
        return super().transform(datastore)
