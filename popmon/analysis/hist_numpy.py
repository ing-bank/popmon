...
def prepare_2dgrid(hist):
    ...
    """
    if hist.n_dim < 2:
        warnings.warn('Input histogram only has {n} dimensions (<2). Returning empty lists.'.format(n=hist.n_dim))
        return [], []
...
def set_2dgrid(hist, xkeys, ykeys):
    ...
    if hist.n_dim < 2:
        warnings.warn('Input histogram only has {n} dimensions (<2). Returning original grid.'.format(n=hist.n_dim))
        return grid
...
def get_2dgrid(hist, get_bin_labels=False):
    ...
    if hist.n_dim < 2:
        warnings.warn('Input histogram only has {n} dimensions (<2). Returning empty grid.'.format(n=hist.n_dim))
        return np.zeros((0, 0))
...
def get_consistent_numpy_2dgrids(hc_list=[], get_bin_labels=False):
    ...
    hist_list = [hc.hist if isinstance(hc, HistogramContainer) else hc for hc in hc_list]
    xkeys = set()
    ykeys = set()
    for hist in hist_list:
        if hist.n_dim < 2:
            raise AssertionError(
                'Input histogram only has {n} dimensions (<2). Cannot compute 2d-grid.'.format(n=hist.n_dim))
...
def get_consistent_numpy_1dhists(hc_list, get_bin_labels=False):
    ...
    nphist_list = []
    for hist in hist_list:
        bin_entries = null_entries if (hist.low is None and hist.high is None) else hist.bin_entries(low, high)
...
def get_consistent_numpy_entries(hc_list, get_bin_labels=False):
    ...
    if not (all_num or all_cat):
        raise AssertionError(
            'Input histograms are mixture of Bin/SparselyBin and Categorize types.'.format(n=hc_list[0].hist.n_dim))
    ...
    entries_list = []
    for hc in hc_list:
        entries = hc.hist.bin_entries(xvalues=labels) if all_num else hc.hist.bin_entries(labels=cat_labels)
...
def check_similar_hists(hc_list, check_type=True, assert_type=used_hist_types):
    ...
    for hist in hist_list:
        if not isinstance(hist, assert_type):
            raise AssertionError('Input histogram type {htype} not of {htypes}.'.format(htype=type(hist),
                                                                                        htypes=assert_type))
    ...
    if not dts.count(dts[0]) == len(dts):
        warnings.warn('Input histograms have inconsistent datatypes: {dts}'.format(dts=dts))
        return False
    ...
    if check_type:
        ...
        if not types.count(types[0]) == len(types):
            warnings.warn('Input histograms have inconsistent class types: {types}'.format(types=types))
            return False
    ...
    if isinstance(hist_list[0], histogrammar.Bin):
        ...
        if not nums.count(nums[0]) == len(nums):
            warnings.warn('Input Bin histograms have inconsistent num attributes: {types}'.format(types=nums))
            return False
        lows = [hist.low for hist in hist_list]
        if not lows.count(lows[0]) == len(lows):
            warnings.warn('Input Bin histograms have inconsistent low attributes: {types}'.format(types=lows))
            return False
        highs = [hist.high for hist in hist_list]
        if not highs.count(highs[0]) == len(highs):
            warnings.warn('Input histograms have inconsistent high attributes: {types}'.format(types=highs))
            return False
    ...
    if isinstance(hist_list[0], histogrammar.SparselyBin):
        ...
        if not origins.count(origins[0]) == len(origins):
            warnings.warn('Input SparselyBin histograms have inconsistent origin attributes: {types}'.format(
                types=origins))
            return False
        bws = [hist.binWidth for hist in hist_list]
        if not bws.count(bws[0]) == len(bws):
            warnings.warn('Input SparselyBin histograms have inconsistent binWidth attributes: {types}'.format(
                types=bws))
            return False
...
def assert_similar_hists(hc_list, check_type=True, assert_type=used_hist_types):
    ...
    """
    similar = check_similar_hists(hc_list, check_type=check_type, assert_type=assert_type)
    if not similar:
