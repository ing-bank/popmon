...
class HistStitcher(Module):
    ...
    def __init__(self, mode='add', time_axis=None, time_bin_idx=None, read_key=None, delta_key=None, store_key=None):
    ...
    def transform(self, datastore):
        ...
        self.logger.info(f'Stitching histograms \"{self.read_key}\" and \"{self.delta_key}\" as \"{self.store_key}\"')
    ...
    def stitch_histograms(self, mode=None, hists_basis=None, hists_delta=None, hists_list=None,
                          time_axis='', time_bin_idx=None):
        """Stitching histograms by first axis.
        ...
        """
        # set stitching mode
        mode = mode if isinstance(mode, str) and mode in self.allowed_modes else self.mode
        time_axis = time_axis if isinstance(time_axis, str) and len(time_axis) > 0 else self.time_axis
        time_bin_idx = time_bin_idx if isinstance(time_bin_idx, (str, int, list, tuple)) else self.time_bin_idx
        ...
        if time_bin_idx is not None:
            ...
            if not isinstance(time_bin_idx, (list, tuple)):
                raise TypeError('time_bin_idx should be a (list of) ordered integer(s) or string(s)')
            dts = [type(tv) for tv in time_bin_idx]
            if not dts.count(dts[0]) == len(dts):
                raise TypeError('time_bin_idxs have inconsistent datatypes: {dts}'.format(dts=dts))
        ...
        if not isinstance(hists_list, list) or len(hists_list) == 0:
            raise TypeError('hists_list should be a filled list of histogram dictionaries.')
        for hd in hists_list:
            if not isinstance(hd, dict) or len(hd) == 0:
                raise TypeError('hists_list should be a list of filled histogram dictionaries.')
        ...
        features_basis = self.get_features(list(hists_basis.keys()))  # basis keys that start with time_axis
        if len(features_basis) > 0:
            # pruning of keys not starting with time_axis
            hists_list[0] = hists_basis = {k: h for k, h in hists_basis.items() if k in features_basis}

        # 1. if there are no basis hists starting with "time_axis:", assume that this the very first batch.
        if len(features_basis) == 0 and time_axis and len(hists_basis) > 0 and time_axis:
            if time_bin_idx is None:
                self.logger.info(f'Inserting basis histograms in axis \"{time_axis}\" at bin index 0.')
                time_bin_idx = [0]
            ...
            for k, hist in hists_basis.items():
                ...
                hists_basis_new[feature] = self._create_hist_with_time_axis(hist, time_bin_idx[0])
            # reset hists_basis
        ...
        if len(hists_delta) == 0 or len(delta_keys) == 0 or len(hists_basis) == 0:
            self.logger.debug('No overlapping delta features. returning pruned hists_basis.')
            return hists_basis

        stitched = dict()

        # 2. if delta(s) do not start with "time_axis:", assume that this is a set of batches without time_axis
        features_delta = self.get_features(list(delta_keys))  # delta keys that start with time_axis
        if len(features_basis) > 0 and len(features_delta) == 0 and len(delta_keys) > 0 and time_axis:
            if time_bin_idx is None or len(time_bin_idx) == 0:
                time_bin_idx = self._generate_time_bin_idx(hists_basis, features_basis, time_axis, len(hists_delta))
                if time_bin_idx is None:
                    raise RuntimeError('Request to insert delta hists but time_bin_idx not set. Please do.')
                self.logger.info(f'Inserting delta histograms in axis \"{time_axis}\" at bin indices {time_bin_idx}.')
            if len(hists_delta) != len(time_bin_idx):
                raise RuntimeError('Not enough time_bin_idxs set to insert delta histograms.')
            for key in list(delta_keys):
                ...
                stitched[feature] = self._insert_hists(hists_basis[feature], hist_list, time_bin_idx, mode)
        ...
        features_overlap = self.get_features(list(overlapping_keys))  # all overlapping keys that start with time_axis
        if len(features_overlap) == 0:
            # no overlap, then return basis histograms
            self.logger.warning('No overlapping basis-delta features. returning pruned hists_basis.')
            return hists_basis
    ...
    def _find_max_time_bin_index(self, hists_basis, features_basis, time_axis):
        ...
        hist_list = [h.hist if isinstance(h, HistogramContainer) else h for h in hists_basis.values()]

        all_sparse = all([isinstance(h, hg.SparselyBin) for h in hist_list])
        all_cat = all([isinstance(h, hg.Categorize) for h in hist_list]) if not all_sparse else False

        max_time_bin_idx = None
        if all_sparse or all_cat:
            max_time_bin_idx = max([max(h.bins.keys()) for h in hist_list if len(h.bins) > 0])
    ...
    def _generate_time_bin_idx(self, hists_basis, features_basis, time_axis, n):
        ...
        """
        assert n > 0
        max_time_bin_idx = self._find_max_time_bin_index(hists_basis, features_basis, time_axis)
    ...
    def _insert_hists(self, hbasis, hdelta_list, time_bin_idx, mode):
        ...
        if len(hdelta_list) != len(time_bin_idx) or len(hdelta_list) == 0:
            raise RuntimeError('hdelta_list and time_bin_idx should be filled and same size.')
        dts = [type(tv) for tv in time_bin_idx]
        if not dts.count(dts[0]) == len(dts):
            raise TypeError('time_bin_idxs have inconsistent datatypes: {dts}'.format(dts=dts))
        ...
        if not hasattr(hbasis, 'bins'):
            raise RuntimeError('basis histogram does not have bins attribute. cannot insert.')
        hdelta_list = [hd.hist if isinstance(hd, HistogramContainer) else hd for hd in hdelta_list]
    ...
    def _create_hist_with_time_axis(self, hist, time_bin_idx):
        ...
        if time_bin_idx is None or not isinstance(time_bin_idx, (str, int)):
            raise TypeError('time_bin_idx not set. should be an (ordered) string or integer.')
        hist = hist.hist if isinstance(hist, HistogramContainer) else hist

        ht = hg.SparselyBin(binWidth=1.0, origin=0.0, quantity=lambda x: x) if isinstance(time_bin_idx, int) \
            else hg.Categorize(quantity = lambda x: x)  # noqa
    ...
    def _stitch_by_update(self, mode, hist_list):
        ...
        """
        hist_list = [hc.hist if isinstance(hc, HistogramContainer) else hc for hc in hist_list]
...
def stitch_histograms(mode=None, hists_basis=None, hists_delta=None, hists_list=None, time_axis=None,
                      time_bin_idx=None):
    """Stitching histograms by first axis.
    ...
    return stitcher.stitch_histograms(mode=mode, hists_basis=hists_basis, hists_delta=hists_delta,
