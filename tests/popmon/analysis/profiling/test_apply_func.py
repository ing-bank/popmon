...
def test_apply_func_module():
    ...
    module = ApplyFunc(
        apply_to_key="to_profile", store_key="profiled", features=["asc_numbers"]
    )
...
def test_rolling_pull_comparer():
    ...
    np.testing.assert_almost_equal(
        p["mae_roll_pull"].values[-1], -1.0811798054391777, 5
    )
...
def test_apply_func():
    ...
    d = apply_func(
        feature=feature, selected_metrics=["a", "b"], df=df, arr=apply_funcs[0]
    )
    ...
    d = apply_func(
        feature=feature, selected_metrics=["a", "b"], df=df, arr=apply_funcs[1]
    )
