...
from popmon.alerting import ComputeTLBounds, traffic_light, pull_bounds, DynamicBounds, TrafficLightAlerts, \
    StaticBounds
from popmon.analysis.apply_func import ApplyFunc
from popmon.analysis.functions import pull, rolling_lr, rolling_mean, expanding_mean, expanding_std
...
def test_apply_dynamic_traffic_light_bounds():
    ...
    m1 = ApplyFunc(apply_to_key="to_profile", features=["asc_numbers"], metrics=['a', 'b'])
    ...
    m2.add_apply_func(pull, suffix='_pull', axis=1, suffix_mean='_mean', suffix_std='_std')
    ...
    tlcs = ['traffic_light_a_red_high', 'traffic_light_a_yellow_high',
            'traffic_light_a_yellow_low', 'traffic_light_a_red_low',
            'traffic_light_b_red_high', 'traffic_light_b_yellow_high',
            'traffic_light_b_yellow_low', 'traffic_light_b_red_low',
            ]
    for c in tlcs:
        assert c in p.columns

    np.testing.assert_almost_equal(p["traffic_light_a_red_high"].values[0], 251.5624903)
    np.testing.assert_almost_equal(p["traffic_light_a_yellow_high"].values[0], 164.96428019)
    np.testing.assert_almost_equal(p["traffic_light_a_yellow_low"].values[0], -65.96428019)
    np.testing.assert_almost_equal(p["traffic_light_a_red_low"].values[0], -152.56249033)
...
def test_apply_static_traffic_light_bounds():
    ...
    np.testing.assert_almost_equal(p["traffic_light_a_yellow_high"].values[1], 164.96428019)
    np.testing.assert_almost_equal(p["traffic_light_a_yellow_low"].values[1], -65.96428019)
    np.testing.assert_almost_equal(p["traffic_light_a_red_low"].values[1], -152.56249033)
...
def test_rolling_window_funcs():
    ...
    m = ApplyFunc(apply_to_key="to_profile", features=["asc_numbers"], metrics=['a', 'b'])
    m.add_apply_func(rolling_mean, suffix='_rolling_3_mean', entire=True, window=3, shift=0)
    m.add_apply_func(rolling_lr, suffix='_rolling_10_slope', entire=True, window=10, index=0)
    m.add_apply_func(rolling_lr, suffix='_rolling_10_intercept', entire=True, window=10, index=1)

    datastore = Pipeline(modules=[m]).transform(datastore)
    feature_df = datastore["to_profile"]["asc_numbers"]

    np.testing.assert_array_almost_equal(feature_df["a_rolling_3_mean"].tolist(), [np.nan] * 2 + list(range(1, 99)))
    np.testing.assert_array_almost_equal(feature_df["a_rolling_10_slope"].tolist(), [np.nan] * 9 + [1.0] * 91)
    np.testing.assert_array_almost_equal(feature_df["a_rolling_10_intercept"].tolist(),
...
def test_report_traffic_light_bounds():
    ...
    rg = SectionGenerator(read_key="to_profile", store_key="section", section_name="Profiles",
                          dynamic_bounds='dtlb', static_bounds='static_tlb')
