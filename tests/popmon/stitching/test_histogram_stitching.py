...
from popmon.hist.filling import make_histograms, get_bin_specs
...
def test_histogram_stitching():
    ...
    hists3 = stitch_histograms(hists_basis=hists1, hists_delta=hists2, time_axis='date', time_bin_idx=[50])
    ...
    hists3 = stitch_histograms(hists_basis=hists2, hists_delta=hists2, time_axis='date', time_bin_idx=[50, 51])
    np.testing.assert_array_equal(sorted(hists3.keys()), sorted(features1 + ['date:age']))
    ...
    hists3 = stitch_histograms(hists_basis=hists2, hists_delta=hists2, time_axis='date', time_bin_idx=50)
    np.testing.assert_array_equal(sorted(hists3.keys()), sorted(features1 + ['date:age']))
    assert hists3['date:age'].entries == 800
    ...
    # add 'date' axis to hists2 (ts=50) and stitch with hists1
    hists3 = stitch_histograms(hists_basis=hists1, hists_delta=hists2, time_axis='date', time_bin_idx=[1],
                               mode='replace')
    np.testing.assert_array_equal(sorted(hists3.keys()), features1)
